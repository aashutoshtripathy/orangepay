...
console.log("File (photograph):", fileNames.photograph);
...
formData.append('datePayment', datePayment);
formData.append('txnId', txnId);  

// Append the photograph file
if (fileNames.photograph) {
  formData.append('photograph', fileNames.photograph);
}

console.log("FormData to be sent:", formData);  // Log the FormData to check
...
for (let pair of formData.entries()) {
  console.log(pair[0] + ': ' + pair[1]);
    ...
    try {
      const response = await axios.post("/api/v1/users/fund-request", formData, {
        headers: {
  ...
  const handleFileChange = (event) => {
    ...
    setFileNames({ ...fileNames, [name]: files[0] }); // Save the actual file object
  };
                  ...
                  <>
                    <CInputGroup className="mb-3 mt-3">
                      ...
                      <CButton color="secondary" onClick={handleButtonClick}>
                        {fileNames.photograph?.name || "Upload Image"}
                      </CButton>
                    </CInputGroup>
                    {errors.photograph && (
