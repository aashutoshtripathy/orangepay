...
const Topup = () => {
  ...
  const createEzetapOrder = async () => {
    ...
    const requestBody = {
      amount: parseFloat(amount), // Amount in INR
      customerName: 'John Doe',
      phone: '9876543210',
      checksum: generateChecksum(amount), // Generate checksum here (implement it based on Ezetap's requirements)
      username: '9810698100',  // Include username for authentication
      password: '770385a0-503f-496f-9ff7-0410b4c90cf3'   // Include password for authentication (if required)
    };
    ...
    try {
      ...
      if (response.data && response.data.qrCode) {
        setQrCode(response.data.qrCode); // Assuming qrCode is Base64 or URL
      } else {
      ...
      <div className="form-container">
        ...
        {qrCode && (
          <div className="qr-code-container">
            ...
            <img
              src={`data:image/png;base64,${qrCode}`} // Assuming qrCode is in Base64 format
              alt="UPI QR Code"
              className="qr-code"
            />
          </div>
